...
from data_util2 import load_graph_classification_dataset
...
def pretrain(model, pooler, dataloaders, optimizer, max_epoch, device, scheduler, num_classes, lr_f, weight_decay_f, max_epoch_f, linear_prob=True, logger=None, 
             mse_mean=mean_loss, mse_none=none_loss):
    ...
    for epoch in epoch_iter:
        ...
        for batch in train_loader:
            ...
            feat = batch_g.ndata["attr"]
            model.train()
            loss, loss_dict, loss_align, rec_loss = model(batch_g, feat, mse_mean, mse_none)
...
def main(args):
    ...
    for i, seed in enumerate(seeds):
        ...
        model.eval()
        test_f1, test_std = graph_classification_evaluation(model, pooler, eval_loader, num_classes, lr_f, weight_decay_f, max_epoch_f, device, mute=False)
...
if __name__ == "__main__":
    args = build_args()
    dataset = ['MUTAG'] # IMDB-MULTI, IMDB-BINARY, MUTAG, COLLAB, PROTEINS, PTC_MR
    for name in dataset:
        args.dataset = name
        if args.use_cfg:
            args = load_best_configs(args, "configs.yml")
        args.seeds = list(range(5))
        main(args)
