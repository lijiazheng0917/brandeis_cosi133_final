...
class PreModel(nn.Module):
    def __init__(
        ...
        self.decoder_deg = nn.Linear(dec_in_dim, 1, bias=False)
    ...
    def mask_attr_prediction(self, g, x, mse_mean, mse_none):
        ...
        with torch.no_grad():
            drop_edge_rate = 0
            if drop_edge_rate > 0:
                use_g_target, masked_edges = drop_edge(pre_use_g, drop_edge_rate, return_edges=True)
            else:
                use_g_target = pre_use_g

            x_t = x.clone()
            new_mask_nodes = mask_nodes.clone()
            num_mask = len(mask_nodes)

            perm = torch.randperm(num_mask, device=x_t.device)
            # if graph use 0.8
            new_mask_nodes = new_mask_nodes[perm[:int(0.8*num_mask)]]

            x_t[new_mask_nodes] = 0.0
            x_t[new_mask_nodes] += self.enc_mask_token

            enc_rep_t, all_hidden_t = self.target(use_g_target, x_t, return_hidden=True)
            rep_t = enc_rep_t
            # rep_t = enc_rep_t + all_hidden_t[0]
            self.momentum_update(self._momentum_rate)

        rep = enc_rep
        # rep = enc_rep + all_hidden[0]
        ...
        # if graph, use all
        online = rep[keep_nodes]
        target = rep_t[keep_nodes]

        # all_rep = rep
        # preds = self.decoder_deg(rep)
        ...
        #degrees = g.ndata['degree'][mask_nodes]
        preds = self.decoder_deg(rep)
        mseloss = nn.MSELoss()
        #deg_loss = mseloss(degrees, preds)

        loss = self.loss_r * rec_loss + self.loss_a * loss_align# + deg_loss * 1e-4

        return loss, loss_align, rec_loss

    def embed(self, g, x):
        rep = self.online(g, x)
        # rep, all_hidden = self.online(g, x, return_hidden=True)
        return rep#  + all_hidden[0]