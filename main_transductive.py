...
from data_util2 import load_dataset, load_transductive_dataset
...
def pretrain(model, linear_model, graph, feat, optimizer, max_epoch, device, scheduler, num_classes, lr_f,
    ...
    for epoch in epoch_iter:
        ...
        if (epoch + 1) % 100 == 0:
            final_acc, estp_acc = node_classification_evaluation(model, graph, x, num_classes, lr_f,
...
def main(args):
    ...
    print(acc_list)
    print(estp_acc_list)
    print(f" final_acc: {final_acc:.4f}±{final_acc_std:.4f}"+f" early-stopping_acc: {estp_acc:.4f}±{estp_acc_std:.4f}")
    # print(f"# early-stopping_acc: {estp_acc:.4f}±{estp_acc_std:.4f}")


if __name__ == "__main__":
    args = build_args()
    dataset = ['citeseer'] # cora, citeseer, wiki, pubmed
    for name in dataset:
        args.dataset = name
        if args.use_cfg:
